    ...
    function readRunProperties(element) {
        return readRunStyle(element).map(function(style) {
            return {
                ...
                // NOTE: font size `w:val` appears to be defaulting to 2x the size set in word. (ex: if set to size 16 in word, xml holds 32)
                // This doesn't appear to be a problem as everything remains the same scale, so leaving this for now. Might need to be tweaked in the future.
