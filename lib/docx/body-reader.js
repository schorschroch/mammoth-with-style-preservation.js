...
var ommlNamespaces = require("../ommlNamespaces.json").ommlNamespaces;
var xml = require("../xml");
...
function BodyReader(options) {
    ...
    var mammothOptions = options.mammothOptions;
    ...
    function readXmlElement(element) {
        if (element.type === "element") {
            ...
            if (handler) {
                var handlerResult = handler(element);
                if (!handlerResult) {
    ...
    var xmlElementReaders = {
        ...
        "w:sym": function(element) {
            return elementResult(new documents.Symbol(element.attributes));
        },
        ...
        "m:oMathPara": readOmmlElements,
        "m:oMath": readOmmlElements
    };
    ...
    function readOmmlElements(element) {
        if (mammothOptions.omml2mathml == "true") {
            var ommlstring = xml.writeString(element, ommlNamespaces);
            ommldom = new xmldom.DOMParser().parseFromString(ommlstring, 'text/xml');
            var mathml = omml2mathml(ommldom);
            return elementResult(new documents.Mathml(mathml.outerHTML));
        } else {
            return false;
        }
