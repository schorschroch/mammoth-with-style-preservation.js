...
function DocumentConversion(options, comments) {
    ...
    var elementConverters = {
        ...
        "symbol": function(element, messages, options) {
            /**
            ...
            /* old type of conversion
            var symchar = '&#x'+element.value.slice(1)+';';
            var attributes = {class: 'fwB-Symbol-' + element.font};
            */
            var symchar = "";
            for (i = 0; i < hexes.length; i++) {
                symchar += String.fromCharCode(parseInt(hexes[i], 16));
            }
            
            var attributes = {class: 'fwB-Symbol'};
            return [Html.freshElement("span", attributes, [Html.pureText(symchar)])];
        },
        ...
        unrecognizedContent: function(element, messages, options) {
            var unrecognizedElementType = element.elType.replace(/\{.*?\}\s?/g, '');
            return Html.freshElement('span', {'data-unrecognized-element': unrecognizedElementType}, [Html.pureText('&nbsp;')]);
        }
    };
    return {
