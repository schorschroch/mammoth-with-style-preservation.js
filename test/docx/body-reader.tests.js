...
test('run has null font size by default', function() {
    var runXml = runWithProperties([]);
...
test('run has font size read from properties', function() {
    var expectedFontSize = '42';
    var fontXml = new XmlElement('w:sz', {'w:val': expectedFontSize});
    var runXml = runWithProperties([fontXml]);

    var run = readXmlElementValue(runXml);
    assert.deepEqual(run.fontSize, expectedFontSize);
});
...
test('run has color read from properties', function() {
    var expectedColor = '3d85c6';
    var fontXml = new XmlElement('w:color', {'w:val': expectedColor});
    var runXml = runWithProperties([fontXml]);

    var run = readXmlElementValue(runXml);
    assert.deepEqual(run.color, expectedColor);
});
